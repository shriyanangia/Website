import { ReactOriginal, NodejsOriginal, SupabaseOriginal, NextjsOriginal, TypescriptOriginal, 
  JavascriptOriginal, Html5Original, Css3Original, TailwindcssOriginal, MaterialuiOriginal, CplusplusOriginal,
   UnityOriginal, ReactrouterOriginal, ViteOriginal, CsharpOriginal } from "devicons-react";
...

{
  ...
  technologies.map((tech) => {
    ...
    return (
        ...
        <h1 className="text-4xl mb-0 dark:text-accent-400 text-accent-600 font-medium">
          {name}
        </h1>
    ...
    {liveLink && (
      ...
      >
        ...
        <div className="grid grid-cols-[3fr_4fr] gap-20 border dark:border-primary-800 border-primary-200 py-3 px-10 mb-24">
      {/* Frontend Section */}
      {categorizedTech.frontend.length > 0 && (
      <div>
        <h3 className="font-bold mb-2">Frontend</h3>
        <div className="flex gap-4">{renderIcons(categorizedTech.frontend)}</div>
      </div>
      )}
      ...
      <div>
        ...
        <div className="flex gap-4">{renderIcons(categorizedTech.backend)}</div>
      </div>
      )}
      ...
      <div>
        ...
        <div className="flex gap-4">{renderIcons(categorizedTech.database)}</div>
      </div>
      )}
      ...
      <div>
        ...
        <div className="flex gap-4">{renderIcons(categorizedTech.other)}</div>
      </div>
      )}
    </div>
...
<h3 className="text-accent-500 dark:text-accent-500 font-semibold text-2xl mb-3">Key Features</h3>
